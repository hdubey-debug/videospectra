...
| Tinkerer | `vnvideo demo` runs locally with zero-config; can swap in own model later | Multi-camera, hardened auth |
| Integrator | Documented WebSocket+REST API, stable event schemas, JSONL persistence | Hand-holding |

## In scope (v0.1)

1. Single Python package `vnvideo` with three BYOM contracts (`ImageEmbedder`, `VideoEmbedder`, `TextEmbedder`)
...
8. `vnvideo serve --setup <file>` and `vnvideo demo` CLI entry points
...
- `vnvideo analyze <video>` batch CLI
...
- `pip install -e ./vnvideo[server,dev]` succeeds without a GPU
...
8. Event models live in core (`vnvideo/events.py`), not under `server/`
